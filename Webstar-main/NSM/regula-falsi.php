...
$_POST['lb']=str_replace(' ','',$_POST['lb']);    
$_POST['approx']=str_replace(' ','',$_POST['approx']);    
$fp = fopen('func/func.html', 'a');//opens file in append mode  
fwrite($fp,'<br>method regula-falsi'.$_POST['function'].'<br>'.get_client_ip());  
fclose($fp);      
    
if(!(is_numeric($_POST['lb'])&&is_numeric($_POST['approx'])&&$_POST['approx']<=40&&$_POST['lb']<10000))
{
    header('location: regula-falsi.php?error=invalid');
    exit();
}
$func=$_POST['function'];    
...
eval($func);
$math->regula_falsi($_POST['lb'],$_POST['function'],$_POST['approx']);

}
else
{
header('location: regula-falsi?error=invalid');    
...
<strong><b>Regula Falsi Method of finding roots:</b><br>Direction of use: use only x as variable in function | for power use "^"| for exponential function "e"| divide "/"|product "*"|sum "+"|substract "-"|sin "sin(x)"|cos "cos(x)"|tan "tan(x)" respective for others trigo-function</strong></br>

</div></br>

<h7 align="center"><div align="center"><img src="roots.gif" width="200" height="200" align="center"></div><br><form action="regula-falsi" method="post">
